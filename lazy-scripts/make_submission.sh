...
#bash catboost.sh fit -m $what.catboost -f full_classif.txt --cd cd.txt -i 1500 $ignore -w 0.001 --loss-function "UserPerObjMetric:alpha=5"
...
bash catboost.sh fit -m $what.catboost -f full_multi.txt --cd cd.txt -i 1000 $ignore -w 0.01 --loss-function MultiClass
...
#what=full.multi
#cat train_multi.txt validate_multi.txt > full_multi.txt
#bash catboost.sh fit -m $what.catboost -f full_multi.txt --cd cd-noweights.txt -i 1000 $ignore-w 0.03 --loss-function MultiClass
#bash catboost.sh calc -m $what.catboost --input-path proper/data/tt_features_003.txt --cd cd-noweights.txt --output-path tmp --prediction-type Probability > /dev/null
#awk -vOFS=',' 'BEGIN{print "answer_id,score"} NR>1{s=0.1*$4+0.5*$5-10*$2-0.1*$3; print $1, (s>0?1:-1)}' tmp > submission.csv


awk -F, '{print $2}' submission.csv | sort | uniq -c
